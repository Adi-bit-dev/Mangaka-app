...
function GenresPage() {
    ...
    const genres = location.state?.genre || "";
    ...
    const searchManga = async (genre) => {
        setLoading(true);
        try {
            const response = await fetch(`http://localhost:3000/search-genre?genre=${encodeURIComponent(genre)}`);
            ...
            if (Array.isArray(data.mangaGenres)) {
                setManga(data.mangaGenres);
            } else {
                setManga([]);
            }
        ...
        <div className="search-root">
            ...
            <div className="search-container">
                <div className="upper-search-section">
                    <div id="search-header">Selected genre:</div>
